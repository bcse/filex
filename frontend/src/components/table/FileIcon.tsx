...
export function FileIcon({ entry }: { entry: FileEntry }) {
  ...
  if (mime.startsWith('video/') || mime.includes('realmedia')) return <Video className="w-4 h-4 text-purple-500" />;
  if (mime.startsWith('audio/')) return <Music className="w-4 h-4 text-pink-500" />;
  if (mime.startsWith('text/') || mime.includes('pdf')) return <FileText className="w-4 h-4 text-red-500" />;
